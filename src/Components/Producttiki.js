...
class ShowProductiki extends Component {
	constructor(props) {
		super(props);
		this.state = {
			products: [],
	...
	async componentDidMount() {
		await axios.get(`http://127.0.0.1:8000/api/get-products`).then((res) => {
	...
	async onSubmitHandle(e) {
		...
		if ($("#inputImage").val().split("\\")[2]) {
			await axios
				.post(`http://127.0.0.1:8000/api/get-products`, fd)
				.then((res) => { });
		}

		await axios
			.post("ttp://127.0.0.1:8000/api/get-products", {
				name: $("#inputName").val(),
